...
## 프로젝트 개요
이 프로젝트는 사용자가 채팅으로 업무를 요청할 수 있는 **지능형 사내 업무 보조 챗봇 시스템**입니다. **AI 에이전트 협업 시스템 LangGraph Swarm**을 통해 여러 전문 에이전트들이 자율적으로 협력하여 사용자의 복합적인 요청을 처리합니다. 기존의 중앙 집중식 라우팅 방식 대신, 각 에이전트가 상황을 판단하여 최적의 협력 파트너를 선택하는 분산 협업 방식을 채택했습니다. 예를 들어, 회사 규정 관련 질문은 **문서 검색 전문 에이전트 RAG Agent**가 **MCP(Model Context Protocol) 툴호출 표준**을 사용하여 처리하고, 데이터 분석 요청은 **데이터 분석 전문 에이전트 Analytics Agent**가 담당하며, 필요시 핸드오프 툴을 통해 다른 에이전트와 협력합니다. 모든 데이터는 **애플리케이션 데이터 설계도 Django 모델**에 따라 체계적으로 저장되며, 프론트엔드는 **실시간 AI 통신 게이트웨이 FastAPI & WebSocket**를 통해 AI 시스템과 매끄럽게 연결되어 AI 답변 생성 과정을 실시간으로 보여줍니다 (스트리밍). 이 시스템은 복잡한 내부 구조를 몰라도 사용자가 AI를 사람과 대화하듯 편안하게 사용할 수 있도록 설계되었습니다.

## 핵심 기능
*   **AI 에이전트 협업 시스템 LangGraph Swarm agent**: 여러 전문 에이전트들이 자율적으로 협력하여 사용자의 복합적인 요청을 처리하는 분산 협업 시스템입니다. 각 에이전트는 자신의 전문 영역에서 작업을 수행하고, 필요에 따라 핸드오프 툴(transfer_to_*)을 통해 다른 에이전트에게 작업을 전달합니다. 중앙 집중식 라우팅 대신 각 에이전트가 상황을 판단하여 최적의 협력 파트너를 선택하는 자율적 협업 방식을 채택했습니다.
*   **문서 검색 전문 에이전트 RAG Agent**: 회사 내부 문서(정책, 매뉴얼, 회의록 등)에 대한 사용자의 질문에 답변합니다. **MCP(Model Context Protocol) 툴호출 표준**을 사용하여 외부 문서 검색 서버와 통신하며, **RAG Fusion 방식**을 통해 원본 질문과 유사 질문들을 생성하여 다각도로 검색한 후 **RRF(Reciprocal Rank Fusion) 점수**로 최적의 문서를 선별합니다. 질문과 관련 있는 문서 조각을 **임베딩**과 **벡터 데이터베이스 Pinecone**를 통해 먼저 검색한 뒤(Retrieval), 그 내용을 바탕으로 답변을 생성합니다(Generation).
*   **데이터 분석 전문 에이전트 Analytics Agent**: 데이터베이스에 저장된 데이터를 분석하고 통찰력을 제공합니다. 사용자의 자연어 질문을 컴퓨터가 이해하는 SQL 쿼리로 변환하고 데이터베이스에서 실행하여 결과를 가져옵니다. 결과는 텍스트로 요약되거나 **chartjs** 코드로 시각화됩니다. (논의를 통해 시계열 예측보다는 이상치 모델링 방향으로 고려되었습니다).
*   **코드 분석 에이전트 Code Agent**: GitHub 저장소나 사내 코드 베이스의 내용을 분석하고 질의응답하는 전문 에이전트입니다. **Pinecone 벡터 검색**을 통해 과거 작업 내역을 찾고, **GitHub API 툴**을 사용하여 실시간 레포지터리 분석을 수행하며, **Context7 MCP 연결**을 통해 최신 라이브러리 정보와 공식 문서에 접근합니다. 코드의 오류 부분을 파악하거나, 특정 함수의 사용 위치나 상호작용하는 파일을 알려주고, 라이브러리 사용법과 코드 변환 기능을 제공합니다.
*   **예측 에이전트 Predict Agent**: 고객 이탈 예측(Customer Churn Prediction)을 전문으로 하는 머신러닝 기반 에이전트입니다. 사용자가 제공한 CSV 형태의 고객 데이터를 분석하여 각 고객의 이탈 가능성을 예측합니다. 사전 훈련된 머신러닝 파이프라인(Scikit-learn 기반)을 사용하여 고객의 서비스 이용 패턴, 요금 정보, 계약 조건 등을 종합적으로 분석한 후 이탈 확률을 제공합니다. 고객 관리 및 마케팅 전략 수립에 활용됩니다. 
...
*   **프론트엔드 채팅 UI**: 사용자가 AI와 직접 소통하고 눈으로 볼 수 있는 '얼굴'입니다. 메시지 입력창, 대화 내용이 보이는 말풍선(메시지 목록), 과거 대화 목록을 보여주는 사이드바, **에이전트 툴 호출 표시**, **핸드오프 과정 시각화**, **차트 렌더링** 등으로 구성됩니다. React(Next.js)의 useState 기능을 사용하여 UI 상태를 관리하고, 서버로부터 받은 메시지와 에이전트 협업 과정을 실시간으로 화면에 그려줍니다.

## 기술 스택
*   **프론트엔드**:
    *   웹 프레임워크: ![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white) (React 기반 풀스택 프레임워크)
    *   UI 컴포넌트: ![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black) (상태 관리 및 컴포넌트 시스템)
    *   스타일링: ![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white) (유틸리티 우선 CSS 프레임워크)
    *   타입스크립트: ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white) (정적 타입 검사)
    *   차트 라이브러리: ![Chart.js](https://img.shields.io/badge/Chart.js-FF6384?style=flat-square&logo=chartdotjs&logoColor=white) (데이터 시각화)
    *   실시간 통신: ![WebSocket](https://img.shields.io/badge/WebSocket-4353FF?style=flat-square&logo=socketdotio&logoColor=white) (AI 스트리밍)
...
*   **백엔드/AI**:
    ...
    *   AI 협업 시스템: ![LangGraph Swarm](https://img.shields.io/badge/LangGraph_Swarm-FF5A5F?style=flat-square&logo=langchain&logoColor=white) (분산 에이전트 협업 및 자율적 핸드오프 시스템)
...
4.  [AI 에이전트 협업 시스템 (LangGraph Swarm)](docs/04_ai_에이전트_총괄_시스템__langgraph_supervisor__.md)
...
*   **기술 스터디**: LangGraph Swarm, MCP (Model Context Protocol), RAG Fusion, LangSmith, Pinecone 사용법 등 핵심 기술에 대한 팀원 간 스터디 진행.
