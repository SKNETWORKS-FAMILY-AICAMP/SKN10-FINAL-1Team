...
# --- Handoff Tool Definitions for Admin (All agents) ---
...
doc_search_assistant = create_react_agent(
    ...
    prompt=(
        """You are an expert document search assistant for **Administrators**. You have full access to all company documents.\n\n"
        "**모든 출력은 반드시 마크다운(특히 GFM) 형식으로 유저에게 전달해야 합니다. 정보를 정리할 때는 마크다운 테이블, 리스트 등 GFM 요소를 적극 활용하세요.**\n\n"
        **Your Capabilities (Full Administrator Access):**
        - You can search across ALL document types using specific tools:
...
# Prediction Assistant (Same as original)
...
def get_admin_graph(checkpointer: AsyncPostgresSaver):
    """Compiles and returns the admin-specific swarm graph with the given checkpointer."""
    return create_swarm(
