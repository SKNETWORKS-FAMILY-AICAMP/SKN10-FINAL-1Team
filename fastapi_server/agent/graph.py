...
_doc_tools = None
_context7_tools = None

async def get_doc_tools():
    """문서 도구들을 lazy loading으로 가져오기"""
    global _doc_tools
    if _doc_tools is None:
        _doc_tools = await client_doc.get_tools()
    return _doc_tools

async def get_context7_tools():
    """Context7 도구들을 lazy loading으로 가져오기"""
    global _context7_tools
    if _context7_tools is None:
        _context7_tools = await client_context7.get_tools()
    return _context7_tools

def create_agents_sync():
    """동기적으로 에이전트들을 생성 (평가 환경용)"""
    # 평가 환경에서는 간단한 도구 목록 사용
    doc_tools = []  # 실제 도구는 런타임에 로드
    context7_tools = []  # 실제 도구는 런타임에 로드
    ...
    return doc_search_assistant, analyst_assistant, predict_assistant, coding_assistant

# 런타임에 실제 도구들을 로드하는 함수
async def create_agents_async():
    """비동기적으로 에이전트들을 생성 (실제 서버 환경용)"""
...
User asks: "langgraph 라이브러리의 reactagent 사용법을 알려줘"
1. ✅ CORRECT: Use `context7_tools` to search for library documentation and usage
2. ❌ WRONG: Do NOT use `github_search_code_documents_with_embedding` first
